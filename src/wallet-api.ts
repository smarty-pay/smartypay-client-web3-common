...
import type { Web3Api, Web3ApiEvent, Web3ApiProvider } from './web3-api';
...
export class WalletApi<EventKey> {
  ...
  async getWalletAddress() {
    if (this.activeWalletApi?.isConnected()) {
  ...
  protected async useApiLock<T>(opName: string, call: (...args: any[]) => Promise<T>): Promise<T | undefined> {
